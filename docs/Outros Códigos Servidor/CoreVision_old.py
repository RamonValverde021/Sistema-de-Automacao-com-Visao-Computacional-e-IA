...
camera = cv2.VideoCapture(1) # 0 para WEbCam do Notebook, 1 para WebCam conectada
cam_width = 320 
cam_height = 240 
...
roi_color = (255, 0, 255)      # Cor inicial da ROI (Magenta = Aguardando) - BGR
...
while True:
    ...
    if object_in_roi:
        if not item_counted:
            ...
            if confidence_score >= CONFIDENCE_THRESHOLD and "Fundo" not in class_name:
                ...
                if current_label == "Coca_Cola":
                    roi_color = (19, 69, 139) # Cor Marrom Escuro
                elif current_label == "Sprite":
                    roi_color = (0, 255, 0) # Cor Verde Limão
                elif current_label == "Fanta_Laranja":
                    roi_color = (0, 165, 255) # Cor Laranja  
                elif current_label == "Fanta_Uva":
                    roi_color = (130, 0, 75) # Cor Indigo
                elif current_label == "Fundo":
                    roi_color = (255, 255, 255) # Cor Branca
                else: # Itens com erro
                    roi_color = (0, 0, 255) # Cor Vermelha
    else:
        ...
        item_counted = False
        current_label = ""
        roi_color = (255, 0, 255) # Cor azul = Aguardando
